...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for _ in range(num_rolls):
        current_roll = dice()
        if current_roll == 1: sow_sad = True
        total_points += current_roll
    return 1 if sow_sad else total_points
    # END PROBLEM 1
...
def tail_points(opponent_score):
    ...
    if tens_digit >= 10: tens_digit = tens_digit % 10
    return 2 * abs(tens_digit - ones_digit) + 1
    # END PROBLEM 2
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    points_scored = 0
    if num_rolls == 0:
        points_scored = tail_points(opponent_score)
    else:
...
def perfect_square(score):
    """Checks if a given score is a perfect square. Returns True if so,
    False otherwise
    """
    square_root = int(score ** .5)
    if square_root * square_root == score:
        return True
    else:
        return False
...
def next_perfect_square(score):
    """Returns the next perfect square for a given score
    """
    square_root = int(score ** .5)
    next_score = (square_root + 1) ** 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    """
    # BEGIN PROBLEM 7
    num_of_rolls = strategy(0, 0)
    for i in range(goal):
        for j in range(goal):
            if strategy(i, j) != num_of_rolls:
                return False
    return True
    # END PROBLEM 7
...
def make_averaged(original_function, total_samples=1000):
    ...
    def averaged_function(*args):
        total_sum = sum(original_function(*args) for _ in range(total_samples))
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM 9
    prev_sum, max_rolls = 0, 0
    averaged_roll_dice = make_averaged(roll_dice, total_samples)
    for i in range(1, 11):
        curr_sum = averaged_roll_dice(i, dice)
        max_rolls = i if curr_sum > prev_sum else max_rolls
        prev_sum = curr_sum
    return max_rolls
    # END PROBLEM 9
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    new_score = square_update(0, score, opponent_score)
