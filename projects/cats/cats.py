...
def pick(paragraphs, select, k):
    ...
    return '' if k >= len(new_paragraphs) else new_paragraphs[k]
    # END PROBLEM 1
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def select(text):
        punc_free_text_list = split(remove_punctuation(text))
        return any([lower(x) in topic for x in punc_free_text_list])
...
def accuracy(typed, source):
    ...
    if len(typed_words) > len(source_words):
        incorrect_words += len(typed_words) - len(source_words)
        typed_words = typed_words[:len(source_words)]
    ...
    return correct_words * 100 / (correct_words + incorrect_words)
    # END PROBLEM 3
